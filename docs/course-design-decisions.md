...
| # | Decision | Choice |
|---|----------|--------|
| 5 | Spec ↔ BDD topology | **Gherkin-native** — feature files *are* the acceptance criteria, and they are what the tests run. **Amended 2026-09-11:** the work item carries the same rules and examples *in full*, so the product person reads the requirement where they work. Neither copy is generated from the other; the kit writes both in one step, and every later step compares them and stops on a difference. The drifting-artifacts failure is handled by that check rather than by keeping the two apart. |
...
| 7 | SDD suite | **Hand-rolled, repo-local, built with the students in class.** A ready-made simple kit ships in the repo (e.g. `course/sdd-kit/`) for anyone stuck — available, not pre-installed. **Amended by 31:** the kit is a composition of primitives, not only markdown prompts. The framework appendix rejected at question 7 is **reinstated** — but now motivated: having built their own process from primitives, students can recognise those primitives inside SpecKit/OpenSpec/nWave. **Built 2026-09-11 as `sdd-lite`** (`skald-sdd/plugins/sdd-lite`, copied here by that repo's `scripts/sync-sdd-lite.sh`): four skills — `sdd-spec`, `sdd-plan`, `sdd-tasks`, `sdd-implement` — plus the `spec-reviewer` sub-agent and the Plane MCP, so the kit demonstrates all three primitives. The `sdd-` prefix is forced: Claude Code owns `/tasks` and `/plan`. |
| 32 | Backlog tool & MCP | **Plane, self-hosted on Coolify**, with its **official MCP server** (`makeplane/plane-mcp-server`). The genuine reason to reach outside the repo, and the course's worked MCP example. See §3b. **Amended 2026-09-11:** Plane is **optional** — without it the kit keeps the work item in `.sdd/<ID>/item.md` and nothing else changes. **One shared account** (`kurs@smidigakademiet.no`) for the whole class in workspace `sdd-kurs`; each pair creates its own project there and tells the kit its name. Attribution is lost by choice — inviting each pair costs setup time that the class does not have, and the git branches say who did what. |
...
| 24 | Claude Cowork | **Dropped 2026-09-11.** The kit runs in Claude Code only; the pair works at the developer's machine, as decision 16 already says. Two reasons, both hard: Cowork does not run on Linux, so this is untestable and undemonstrable here; and a custom connector can only carry a token if the organisation has the request-headers beta, which this account does not — and the Plane connector additionally requires an `x-workspace-slug` header, a custom name Anthropic must approve. |
